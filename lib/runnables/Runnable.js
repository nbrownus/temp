...
Runnable.prototype.resetTimeout = function () {
    ...
    if (this._timeout) {
        this.timer = setTimeout(function () {
            self.complete(new Error('timeout of ' + self._timeout + 'ms exceeded'), Constants.RUN_STATE.TIMEOUT)
        }, self._timeout)
    }
}
...
Runnable.prototype.run = function () {
    ...
    self.dependencies.some(function (runnable) {
        if (runnable.error) {
            self.state = Constants.RUN_STATE.HOOK_FAILURE
            //TODO: Better errors, which hook type?
            self.error = new Error('Hook failed:', runnable.fullTitle('/'))
            return false
        }
    })
    ...
    if (self.skip) {
        return self.complete(undefined, Constants.RUN_STATE.SKIPPED)
    }
 ...
 * @param {Number} [state] An override to the normal state value. Useful for timeout and skip
 */
Runnable.prototype.complete = function (error, state) {
    var self = this

    if (self.state !== Constants.RUN_STATE.RUNNING) {
        if (self.state === Constants.RUN_STATE.TIMEOUT) {
            return
        }
    ...
    if (error) {
        ...
        self.error = error
        self.state = state || Constants.RUN_STATE.FAILURE
    } else {
        self.state = state || Constants.RUN_STATE.SUCCESS
    }
