...
Runnable.prototype.resetTimeout = function () {
    ...
    if (this._timeout) {
        this.timer = setTimeout(function () {
            self._complete(new Error('timeout of ' + self._timeout + 'ms exceeded'))
            self.timedOut = true
        }, self._timeout)
    }
}
...
Runnable.prototype.run = function () {
    ...
    throw new Error('Subclass did not implement #run')
}
...
/**
 * Actually runs the function registered for this object
 *
 * @param {function} callback A function to call when the test function is complete
 */
Runnable.prototype.actualRun = function (callback) {
    var self = this
