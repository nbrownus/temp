...
/**
 * Getter/setter marks this runnable to be skipped
 *
 * @returns {Runnable|Boolean} This for chaining if setting, whether or not this is runnable will be skipped if getting
 */
Runnable.prototype.skip = function (skip) {
    if (!arguments.length) {
        return this._skip
    }
 ...
 */
Runnable.prototype.only = function (only) {
    if (!arguments.length) {
        return this._only
    }
 ...
 *
 * @returns {Runnable} The cloned runnable
 */
Runnable.prototype.clone = function (parent) {
    var runnable = new Runnable(this.title, this.func)

    runnable.timeout(this.timeout())
    runnable.skip(this._skip)
    runnable.only(this._only)
    runnable.exclusivity = this.exclusivity
    runnable.parent = parent || this.parent
