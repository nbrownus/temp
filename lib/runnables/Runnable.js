 ...
 * @param {Number} [failureState] An override to the normal failure run state value. Useful for timeout
 */
Runnable.prototype.complete = function (error, failureState) {
    var self = this
    ...
    if (error) {
        ...
        self.error = error
        self.state = failureState || Constants.RUN_STATE.FAILURE
    } else {
        self.state = Constants.RUN_STATE.SUCCESS
    }
