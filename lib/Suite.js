...
Suite.prototype.prepare = function (rootDependencies) {
    ...
    self.runnables.beforeAll.forEach(function (runnable, index) {
        //TODO: This includes things before us as well as us :(
        runnable.addRunnableDependants(self.allRunnables().slice(index))
        self._setRunnableProperties(runnable, self._nonExclusives)
    })
    self._closeNonExclusives()

    //Run the actual tests, giving them every beforeEach and afterEach
    var beforeEachNonExclusives = []
      , afterEachNonExclusives = []

    self.runnables.tests.forEach(function (runnable) {
        self.runnables.beforeEach.forEach(function (hook) {
            var newHook = hook.clone()
            newHook.parent = self
            if (runnable.skip) {
                newHook.skip = true
            }
            newHook.addRunnableDependants(runnable)
            self._setRunnableProperties(newHook, beforeEachNonExclusives)
            runnable.addDependencies(newHook)
        })

        if (beforeEachNonExclusives.length > 0) {
        ...
        /*
        self.runnables.afterEach.forEach(function (hook) {
            var newHook = hook.clone()
            if (runnable.skip) {
                newHook.skip = true
            }
            self._setRunnableProperties(newHook, afterEachNonExclusives)
        })
        self._closeNonExclusives(afterEachNonExclusives)
        */
    })
    self._closeNonExclusives()
...
Suite.prototype._setRunnableProperties = function (runnable, nonExclusives) {
    var self = this
    ...
    switch (runnable.exclusivity) {
        ...
        case Constants.EXCLUSIVITY.LOCAL:
            if (nonExclusives.length) {
                runnable.addDependencies(nonExclusives)
            } else {
