...
Suite.prototype._createTestContainer = function (runnable) {
    ...
    if (runnable.exclusivity) {
        suite.exclusivity = Math.max(suite.exclusivity, runnable.exclusivity)
    } else {
        suite.exclusivity = Math.max(suite.exclusivity, suite.testExclusivity)
    }
