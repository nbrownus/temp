...
Suite.prototype.prepare = function () {
    ...
    self.suiteDependencies.forEach(function (suite) {
        //TODO: This is double adding shit for locally exclusive testContainers :(
        self.nextTestDependencies = self.nextTestDependencies.concat(suite.nextTestDependencies.slice())
    })

    //console.log('Suite', self.title, 'depends on', self.allTitles(self.suiteDependencies))
    //console.log('Next deps', '%%%%', self.title, ':::::', self.allTitles(self.nextTestDependencies))
    ...
    self._closeNonExclusives()
    self._prepareSuites(testSuites, true)

    //Build up all child suites
    self._prepareSuites(self.suites, false)
...
Suite.prototype._prepareSuites = function (suites, prepare) {
    var self = this
      , nonExclusiveSuites = []
 ...
 *
 * @param {Array.<Suite>} dependencies An array of suite dependencies
 */
...
Suite.prototype._createTestContainer = function (runnable) {
    ...
    suite.exclusivity = Constants.EXCLUSIVITY.NONE
    suite.parent = self
...
Suite.prototype._completePrep = function () {
    ...
    if (self.suites.length) {
        ...
        console.log('Closing', self.title, 'lllllll', self.allTitles(self.nextTestDependencies))
    }
