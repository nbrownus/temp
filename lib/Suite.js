...
Suite.prototype.prepare = function () {
    ...
    self.hasOnlyTest = self.hasOnly(TEST_TYPE.NORMAL)
    ...
    if (self.tests.beforeAll.length) {
        hasOnly = self.hasOnly(TEST_TYPE.BEFORE_ALL)
    ...
    if (self.tests.afterAll.length) {
        hasOnly = self.hasOnly(TEST_TYPE.AFTER_ALL)
...
Suite.prototype._prepareSuites = function (suites) {
    ...
    suites.forEach(function (suite) {
        ...
        suite.on('test', function (test) {
            if (test.exclusivity === EXCLUSIVITY.GLOBAL) {
...
/**
 * Gets all titles for an array of suites or tests
 *
 * @param {Array.<Test|Suite>} items An array of suites or tests to get titles for
...
Suite.prototype._createTestContainer = function (test) {
    ...
    function addHooks (type) {
        var adder = (type === TEST_TYPE.BEFORE_EACH) ? 'addBeforeAll' : 'addAfterAll'
    ...
    addHooks(TEST_TYPE.BEFORE_EACH)
    addHooks(TEST_TYPE.AFTER_EACH)
...
Suite.prototype._setTestProperties = function (test, hasOnly) {
    ...
    if (test.exclusivity === EXCLUSIVITY.NONE) {
 ...
 * If the type is Contants.TEST_TYPE.NORMAL it will check all child suites
 *
...
Suite.prototype.hasOnly = function (type) {
    ...
    if (type === TEST_TYPE.NORMAL && !hasOnly) {
