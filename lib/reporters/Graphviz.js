...
GraphViz.prototype.printNodes = function () {
    ...
    self.pocha.allRunnables.forEach(function (runnable) {
        ...
        if (runnable instanceof runnables.Test) {
            //TODO: Fix this
            runnable.hooks.before.forEach(function (hook) {
                allRows.push(hook.fullTitle('/'))
            })

            allRows.push(runnable.title)

            runnable.hooks.after.forEach(function (hook) {
                allRows.push(hook.fullTitle('/'))
            })

        } else if (runnable instanceof runnables.AllHook) {
        ...
        if (!(runnable instanceof runnables.AllHook) && runnable.parent) {
