 ...
 * @constructor
 */
var BDD = function (pocha, context) {
    this.pocha = pocha
    this.suites = [pocha.rootSuite]
...
BDD.prototype.describe = function (title, func) {
    var suite = new Suite(title, this.pocha)
...
BDD.prototype.before = function (title, func) {
    if (arguments.length === 1) {
        func = title
        title = 'Before all hook ' + (this.suites[0].runnables.beforeAll.length + 1)
    }

    var suite = this.suites[0]
      , hook = new runnables.AllHook(title, func)
...
BDD.prototype.after = function (title, func) {
    if (arguments.length === 1) {
        func = title
        title = 'After all hook ' + (this.suites[0].runnables.afterAll.length + 1)
    }
...
BDD.prototype.beforeEach = function (title, func) {
    if (arguments.length === 1) {
        func = title
        title = 'Before each hook ' + (this.suites[0].runnables.beforeEach.length + 1)
    }

    var suite = this.suites[0]
      , hook = new runnables.EachHook(title, func)
...
BDD.prototype.afterEach = function (title, func) {
    if (arguments.length === 1) {
        func = title
        title = 'After each hook ' + (this.suites[0].runnables.afterEach.length + 1)
    }
